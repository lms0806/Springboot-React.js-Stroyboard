...
import com.cos.book.model.user.dto.UserDto;
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!method.equals("POST")) {
			...
			try {
				...
				User personEntity = personRepository.findByUsernameAndPassword(person.getUsername(),
						person.getPassword());
				...
				if (personEntity == null) {
					...
					String jwtToken = JWT.create().withSubject("토큰제목")
							...
							.withClaim("id", personEntity.getId()).sign(Algorithm.HMAC512(JwtProps.secret));

					resp.addHeader(JwtProps.header, JwtProps.auth + jwtToken);
					
					UserDto<User> userDto = new UserDto<>(1, "success", personEntity);
					String cmRespDtoJson = om.writeValueAsString(userDto);
